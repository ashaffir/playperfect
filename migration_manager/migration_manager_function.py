...
def migration_manager(request):
    ...
    migrate_chunk_url = (
        "https://us-central1-playperfect-432410.cloudfunctions.net/migrate_chunk"
    )
    errors = []
    ...
    def send_chunk_request(chunk_index):
        ...
        try:
            response = requests.post(
                migrate_chunk_url,
                json={"start_row": start_row, "chunk_size": chunk_size},
            )
            ...
            logging.error(
                f"Failed to trigger migration for chunk {chunk_index + 1}: {str(e)}"
            )
            return {"chunk_index": chunk_index, "start_row": start_row, "error": str(e)}

    # Using ThreadPoolExecutor to run requests in parallel
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_REQUESTS) as executor:
        futures = [
            executor.submit(send_chunk_request, chunk_index)
